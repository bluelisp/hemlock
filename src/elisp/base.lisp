...
(cl:defun emacs-version (&optional here)
  (unless elisp-internals:*emacs-version*
    (let ((sym 'hemlock-ext:key-event))
      (let ((package (symbol-package sym))
            (ext-package (find-package "EXT"))
            (hemlock-package (find-package "HEMLOCK-EXT")))

        (setf elisp-internals:*emacs-version*
              (format nil "Hemlock (~a) (CL elisp ~a) ~a"
                      (if (eq package ext-package) "CMU CL" "Portable Hemlock")
                      elisp-internals:*cl-elisp-release*
                      hemlock-ext:*hemlock-version*)))))
  elisp-internals:*emacs-version*)

(cl:defmacro version (&optional arg)
  `(emacs-version ,@arg))
