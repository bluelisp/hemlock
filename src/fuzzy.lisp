...
(defun fuzzy-completion-selected (original-string completion)
...
;;; helper functions, also from swank
...
;;;
(defvar *fuzzylist-buffer* nil)

;;; This is the cleanup method for deleting *fuzzylist-buffer*.
;;;
(defun delete-fuzzylist-buffers (buffer)
  (when (eq buffer *fuzzylist-buffer*)
    (setf *fuzzylist-buffer* nil)
    (setf *fuzzylist-entries* nil)))
...
(defcommand "Fuzzylist Find Definition" (p)
  "" ""
  (declare (ignore p))
  (let ((entry (fuzzylist-entry-from-mark (current-point))))
    (when entry
      (change-to-definition entry))))

(defcommand "Fuzzylist Pick" (p)
  "" ""
  (declare (ignore p))
  (let ((entry (fuzzylist-entry-from-mark (current-point))))
    (when entry
      (fuzzylist-quit-command nil)
      (delete-window-command nil)
      (%fuzzy-insert-completion (fuzz-completed-string entry)))))

(defun refresh-fuzzylist (buf entries)
...
(defun make-fuzzylist-buffer (entries)
  (let ((buf (or *fuzzylist-buffer*
                 (make-buffer "*Fuzzylist*" :modes '("Fuzzylist")))))
    (setf *fuzzylist-buffer* buf)
    (refresh-fuzzylist buf entries)
    (let ((fields (buffer-modeline-fields *fuzzylist-buffer*)))
      (setf (cdr (last fields))
            (list (or (modeline-field :fuzzylist-cmds)
                      (make-modeline-field
                       :name :fuzzylist-cmds :width 18
                       :function
                       #'(lambda (buffer window)
                           (declare (ignore buffer window))
                           "  Type ? for help.")))))
      (setf (buffer-modeline-fields *fuzzylist-buffer*) fields))
...
(defcommand "Fuzzylist Help" (p)
  ...
  "Show this help."
  (declare (ignore p))
  (describe-mode-command nil "Fuzzylist"))
